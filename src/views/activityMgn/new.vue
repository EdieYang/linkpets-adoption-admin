    ...
    <div style="width:100%;">
      <el-card v-loading="pageLoading">
        <el-form ref="form"
          ...
          <el-form-item label="活动圈子"
                        prop="groupId">
            <el-select v-model="form.groupId"
                       placeholder="请选择活动圈子"
                       style="width:100%;">
              <el-option v-for="item in circles"
                         :key="item.groupId"
          ...
          <el-form-item label="参加活动时间"
            ...
            <el-button type="primary"
                       round=""
                       size="small"
                       @click="addTime">添加活动时间</el-button>
          </el-form-item>
          ...
          <el-form-item label="活动地址"
            ...
            <el-input v-model="form.activityAddress"
                      placeholder="请输入活动地址"></el-input>
          </el-form-item>
          <el-form-item label="消耗积分"
                        prop="activityCost">
            <el-input-number v-model="form.activityCost" :min="0" :step="1" step-strictly size="medium"></el-input-number>
          </el-form-item>
          <el-form-item label="获得积分"
                        prop="activityPoint">
            <el-input-number v-model="form.activityPoint" :min="0" :step="1" step-strictly size="medium"></el-input-number>
          </el-form-item>
          <el-form-item label="活动banner图"
            ...
            <el-input v-model="form.activityBanner"
                      v-show="false"></el-input>
          </el-form-item>
          <el-form-item label="活动管理员微信"
                        prop="customerSupport">
            <el-input v-model="form.customerSupport"
                      placeholder="请输入活动管理员微信"></el-input>
          </el-form-item>
          <el-form-item label="是否验证身份证"
                        prop="activityShouldVerify">
            <el-switch
              v-model="form.activityShouldVerify">
            </el-switch>
          </el-form-item>
          <el-form-item label="是否增加表单"
                        prop="activityShouldQuestionnaire">
            <el-switch
              v-model="form.activityShouldQuestionnaire">
            </el-switch>
            <el-select v-if="form.activityShouldQuestionnaire"
            ...
            <el-button v-if="form.activityShouldQuestionnaire"
                       ...
                       round=""
                       size="small"
                       @click="addQuestionnaire">添加问卷</el-button>
          </el-form-item>
          <el-form-item label="活动详情"
                        prop="activityContent">
                        <div>
            <d2-ueditor :default-config="config" v-model="form.activityContent" style="width:100%"/>
            </div>
          </el-form-item>
          ...
          <el-button class="form-sumbit"
                     @click="cancel"
                     style="margin-right: 20px">取 消</el-button>
        </el-form>
        <br>
      </el-card>
    </div>
  ...
  data () {
    ...
    return {
      ...
      formRules: {
        ...
        joinTime: [
          { required: true, validator: checkTime, message: '请选择参加活动时间', trigger: 'change' },
        ],
        applyTime: [
  ...
  methods: {
    activityDetail () {
      ...
      activityDetail(data).then(res => {
        ...
        this.form.activityShouldQuestionnaire = this.form.activityShouldQuestionnaire === '1'
        this.fileList = [{url:this.form.activityBanner}]
        this.form.activityTime = [this.form.activityStartTime,this.form.activityEndTime]
        this.form.applyTime = [this.form.activityRegisterStartTime,this.form.activityRegisterEndTime]
    ...
    adoptNew () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.form.activityStartTime = this.formatTime(this.form.activityTime[0])
          this.form.activityEndTime = this.formatTime(this.form.activityTime[1])
          this.form.activityRegisterStartTime = this.formatTime(this.form.applyTime[0])
          this.form.activityRegisterEndTime = this.formatTime(this.form.applyTime[1])
          let pickTime = ''
          this.form.joinTime.map(item => {
            pickTime = pickTime.concat(this.formatTime(item[0])).concat('-').concat(this.formatTime(item[1])).concat(',')
          }) 
          this.form.activityPickTime = pickTime.substring(0, pickTime.length - 1)
          this.form.activityShouldVerify = this.form.activityShouldVerify ? '1' : '0'
          this.form.activityShouldQuestionnaire = this.form.activityShouldQuestionnaire ? '1' : '0'
    ...
    handleSuccess (response, file, fileList) {
      this.form.activityBanner = "https://pic.linchongpets.com/" + file.response.data
